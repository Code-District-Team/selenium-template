...
class payment_processing:
    ...
    def payment_processing(self):
        card_number = WebDriverWait(self.driver, 30).until(EC.element_to_be_clickable(payment_page_locators.card_number_input_field))
        card_number.send_keys(payment_data.credit_card_number)
        expiry_date = WebDriverWait(self.driver, 30).until(EC.element_to_be_clickable(payment_page_locators.expires_input_field))
        expiry_date.send_keys(payment_data.expiry_date)
        cvv_number = WebDriverWait(self.driver, 30).until(EC.element_to_be_clickable(payment_page_locators.security_code_input_field))
