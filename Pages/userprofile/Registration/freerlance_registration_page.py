...
class FreelanceRegistration:
    ...
    def freelance_info(self):
        description_text = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.description_box))
        description_text.click()
        description_text.send_keys("This is an automation text")
        hourly_rate = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.hourly_rate))
        hourly_rate.send_keys("400")
        currency_dropdown = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.currency_dropdown))
        currency_dropdown.click()
        select_currency_type = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.select_currency))
        select_currency_type.click()
        minimumProjectSize = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.minimum_project_size))
        minimumProjectSize.send_keys(RegistrationTestData.minimum_project_price)
        availability_dropdown = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.availability_dropdown))
        availability_dropdown.click()
        select_availablity = WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located(registrationLocators.select_availability))
        select_availablity.click()
    def click_to_continue(self):
        continue_button = WebDriverWait(self.driver, 40).until(EC.element_to_be_clickable(registrationLocators.continue_button_freelance))
        continue_button.click()
    def create_account_button(self):
        create_account_button = WebDriverWait(self.driver, 40).until(EC.element_to_be_clickable(registrationLocators.create_account_button))
    ...
    def continue_to_payment(self):
        continue_to_payment_button = WebDriverWait(self.driver, 40).until(EC.element_to_be_clickable(registrationLocators.continue_button))
