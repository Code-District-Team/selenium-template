...
class businesstrade:
    ...
    def navigate_to_trade_tab(self):
        profileAvatar = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.profileAvtar))
        profileAvatar.click()
        accountSettings = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.accountSettings))
        accountSettings.click()
        businessTab = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.businessTab))
        businessTab.click()
        tradeTab = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.tradeTab))
        tradeTab.click()
        time.sleep(5)

    def add_brands(self):
        time.sleep(3)
        addBrandButton = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.addMoreBrands))
        addBrandButton.click()
        selectBrand = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.addMoreBrandsDropdown))
        selectBrand.click()
        selectBrandOption = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.addMoreBrandsDropdownOption))
        selectBrandOption.click()
        saveBrand = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.saveButton))
        saveBrand.click()
        self.driver.execute_script("arguments[0].scrollIntoView();", addBrandButton)

    def addOriginAddress(self):
        addOrigin = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.addOriginAddress))
        addOrigin.click()
        countryDropdown = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.choseCountry))
        countryDropdown.click()
        time.sleep(5)
        countryDropdown.send_keys("Afghanistan")
        countryDropdown.send_keys(Keys.ENTER)
        stateDropdown = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.chooseState))
        stateDropdown.click()
        stateDropdownOption = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.chooseStateOption))
        stateDropdownOption.click()
        cityInput = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.cityInput))
        cityInput.send_keys(businessInfoData.city)
        zipCode = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.zipCodeInput))
        zipCode.send_keys(businessInfoData.zipCode)
        streetAddress = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.streetAddressInput))
        streetAddress.send_keys(businessInfoData.streetAddress)
        saveOriginAddress = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.saveButton))
        saveOriginAddress.click()

    def addCurrency(self):
        addCurrency = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.addAcceptedCurrencyButton))
        addCurrency.click()
        addCurrencyDropdown = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.addMoreBrandsDropdown))
        addCurrencyDropdown.click()
        addCurrencyOption = WebDriverWait(self.driver, 100).until(
            EC.element_to_be_clickable(tradelocators.addAcceptedCurrencyButtonOption))
        addCurrencyOption.click()
        saveCurrency = WebDriverWait(self.driver, 100).until(EC.element_to_be_clickable(tradelocators.saveButton))
        saveCurrency.click()

    def addNearestPort(self):
        addNearestPort = WebDriverWait(self.driver, 100).until(
